...
export default class ManpowerDWTR extends Component {
  constructor() {
    ...
    validateTeamForm =() => {
      if ((typeof this.state.team === 'undefined' || this.state.team == '')) {
        toast.error('Please select team', { autoClose: 3000 });
        return false;
      }
      if ((typeof this.state.workerCount === 'undefined' || this.state.workerCount == '')) {
    ...
    handleSubmit = () => {
      if (this.validateTeamForm() === true) {
        const manpowerList = {
          uniquieId: Date.now(),
          value_team: this.state.team,
          text_team: this.state.team_text,
    ...
    render() {
      ...
      return (
        <div className="manPowerSelection">
          ...
          <div className="row">
            <div className="col-xs-6"><label>No.of Workers</label></div><div className="col-xs-6">
              <CustInput
                type="number"
                size="4"
                name="workerCount"
